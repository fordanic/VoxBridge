...
- [x] 1. Create a `BaseSTT` interface/class with a method `transcribe(audio_chunk) -> str`.
- [x] 2. Implement a `DummySTT` returning a hardcoded string for testing.
- [x] 3. Update the main application flow to:
  - Read config.yaml
  - Instantiate the chosen STT engine
  - Fall back to DummySTT if none is set
- [x] 4. Implement automatic gain control (AGC) if supported by the STT engine
- [x] 5. Add support for custom dictionaries configuration
